function setup(mdl)
    ...
    elseif mdl.stype == 3
        mdl.eq=mdl.conf.ode3D;
        mdl.vf_=mdl.conf.vf3D;
        if isempty(mdl.byname)
            mdl.params=mdl.conf.params3D;
        end
        mdl.parnames=mdl.conf.parnames3D;
        mdl.IC=mdl.conf.IC2D;
        mdl.vfrange=mdl.conf.vfrange2D;
        mdl.nc1=mdl.conf.nc1_3D;
        mdl.nc2=mdl.conf.nc2_3D;
    else
