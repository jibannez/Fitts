function setup(mdl)
    ...
    elseif mdl.stype==2
        if strcmp(mdl.byname,'em')
            ...
            %mdl.fitfcn= @(p, y)...
            %[-1*( p(1) + p(3)*sin(2*(y(:,1)'-p(7))+p(9)) + p(5)*cos(4*(y(:,1)'-p(7))))+ p(11)*sin(y(:,2)'-y(:,1)') ;...
            % -1*( p(2) + p(4)*sin(2*(y(:,2)'-p(8))+p(10))+ p(6)*cos(4*(y(:,2)'-p(8))))+ p(11)*sin(y(:,1)'-y(:,2)')]';
        end
        mdl.fitfcn= @(p, y)...
        [-0.8*( p(1) + p(3)*sin(2*(y(:,1)'-p(7))+p(9)) + p(5)*cos(4*(y(:,1)'-p(7))))+ p(11)*sin(y(:,2)'-y(:,1)') ;...
