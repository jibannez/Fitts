...
xnorm=x/max(abs(x));
vnorm=v/max(abs(v));
xnorm_hist=get_ts_histogram(x,tr.ts.peaks,tsL);
xnorm_hist=xnorm_hist/max(abs(xnorm_hist));
vnorm_hist=get_ts_histogram(v,tr.ts.peaks,tsL);
vnorm_hist=vnorm_hist/max(abs(vnorm_hist));
xnorm_phhist=get_ph_histogram(x,ph,phL);
xnorm_phhist=xnorm_phhist/max(abs(xnorm_phhist));
vnorm_phhist=get_ph_histogram(v,ph,phL);
vnorm_phhist=vnorm_phhist/max(abs(vnorm_phhist));
omega_phhist=get_ph_histogram(omega,ph,phL);

%ph_phhist=resize_vector(tr.ts.([h,'ph_phhist']),phL);
%omega_phhist=resize_vector(-tr.ts.([h,'omega_phhist']),phL);
...
omega_hist=resize_vector(-tr.ts.([h,'omega_hist']),tsL);
