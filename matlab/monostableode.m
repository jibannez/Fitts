function dydt = monostableode(t,y,kk,w1,w2,a1,a2,b1,b2,c1,c2,d1,d2,alpha)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Model equations:
    ...
    Imax=1.0;    %Maximal pulse intensity
    dmax=0.15;   %Maximal pulse duration
    sg=0.025;    %Pulse intensity standart deviation
    phthr=pi/8;  %Phase threshold to start a pulse
    tstart=2*sg; %Advance in time to start a pulse
    ...
    dydt=[-( w1 + a1*sin(2*(y(1)-c1)+d1) + b1*cos(4*(y(1)-c1)) + alpha*sin(y(2)-y(1)) ) ;...
          -( w2 + a2*sin(2*(y(2)-c2)+d2) + b2*cos(4*(y(2)-c2)) + alpha*sin(y(1)-y(2)) + I)];
