function pguess=fit_trial2D(mdl,tr,do_plot,alphac)
    if nargin<4, alphac=0.0;end
    ...
    for h=1:2
        ...
        if tr.info.([hands{h},'ID']) < 3
            pguess{ h }=sqrt(wfexp{h}^2+(3*pguess{idx})^2);
        else
            pguess{ h }=sqrt(wfexp{h}^2+(1.5*pguess{idx})^2);
        end
