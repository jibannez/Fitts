...
function  plot_time_series(ts,ax)
    ...
    if nargin==1, 
        ...
        plot(ts.Ljerknorm, 'c');        
        plot(ts.Lph/pi,'k--');
        plot(-ts.Lomega/max(abs(ts.Lomega)),'g')        
        legend({'Lxnorm','Lvnorm','Lanorm','Ljerknorm','Lphnorm','Lomeganorm'})
        ...
        plot(ts.Rjerknorm, 'c');
        plot(ts.Rph/pi,'k');
        plot(-ts.Romega/max(abs(ts.Romega)),'g')
        legend({'Rxnorm','Rvnorm','Ranorm','Rjerknorm','Rphnorm','Romeganorm'})
