classdef GroupDID6PlotConfig < handle
    properties
        data
        data_scatter
        vname
    ...
    methods
        ...
        %Constructor
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function gpc = GroupDID6PlotConfig(bi,factors,savepath,vname,mode)
            ...
            gpc.mode=mode;
            plotdefaults(gpc);
            get_barplotdid6_props(gpc,bi)
            if gpc.mode==3
                get_lineplot_props(gpc,{'ss','did'},'6levels2')
                %get_lineplot_props(gpc,{'did','ss'},'6levels2')
            end
        end
        ...
        function gap = get_gap(gpc,f1,f2)
            ...
            gap=1+(gpc.ss+0.5)*(f2-1)+floor((f2-1)/2);%+((gpc.ss+0.5)*gpc.franges(2)-1)*(f1-1);
        end
        ...
        function get_xlabels_pos(gpc)
           ...
           for i=1:gpc.franges(1)
               if gpc.franges(2)==2
                   gpc.xlabels_pos(i,1)=gpc.get_gap(i,1)+2;
               else
