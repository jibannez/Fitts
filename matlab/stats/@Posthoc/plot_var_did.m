...
function [bar_data,gp]=plot_interaction(phoc,data,factors,vname,ax,do_legend,do_ylabel)
    ...
    %Get graphic properties of factors        
     gp=plotdefaults();
     gp.vname=phoc.vname;
     gp=get_lineplot_props(gp,factors,phoc.ds.DIDmode);
     gp=get_props(gp,factors,phoc);
    ...
    hold on
    for i=1:f1  
        h{i}=errorbar(ax,gp.xticks+mv(i),bar_data(i,:,1),bar_data(i,:,2),gp.linespec{i},'MarkerFaceColor','k','LineWidth',gp.linewidth(i));     
    end
    
    %Limits, ticks and box
    if phoc.ds.do_latex
        format_ticks(gca,gp.xticklabels,gp.yticklabels,gp.xticks,gp.yticks,[],[],[],'interpreter','latex','FontName',phoc.ds.fontname,'FontSize',phoc.ds.fontsize);
    else
        format_ticks(gca,gp.xticklabels,gp.yticklabels,gp.xticks,gp.yticks,[],[],[],'FontName',phoc.ds.fontnamelatex,'FontSize',phoc.ds.fontsize);
    end
    add_minor_ticks(gp.yminorticks,[0,0.1],'y');
    set(gca,'box','on'); 
    ylim(gca,gp.ylim)
    ...
    if do_ylabel
        if phoc.ds.do_latex
            ylabel(['$',vname,'$'],'rotation',0,'interpreter','latex','FontName',phoc.ds.fontnamelatex,'FontSize',phoc.ds.fontsize+2);
        else            
            ylabel(vname,'rotation',0,'FontName',phoc.ds.fontname,'FontSize',phoc.ds.fontsize+2);
        end
...
function gp = get_props(gp,factors,ds)
